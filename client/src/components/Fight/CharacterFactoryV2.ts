...
export default class CharacterFactoryV2 {
  static characterData: any;
  static petData = new Map<number, any>();

  static create(scene: Phaser.Scene, role: FightRole): Character {
    if (CharacterFactoryV2.characterData === undefined || CharacterFactoryV2.characterData[role.avatar] === undefined) {
      console.error(CharacterFactoryV2.characterData);
      throw Error('Invalid character data ' + role.avatar);
    }

    const { index, isOnOffense, avatar, hp, maxHp, mp, maxMp } = role;
    const character = new Character(scene);
    const characterData = CharacterFactoryV2.characterData[role.avatar];
    ...
    if (role.pet) {
      ...
      character.pet.animations = CharacterFactoryV2.petData?.get(role.pet)?.animations;
    }
  ...
  static load(scene: PreloadScene, role: FightRole) {
    ...
    const { weaponAnimations, secretTechnique, name } = CharacterFactoryV2.characterData[role.avatar];
    ...
    if (role.pet) {
      CharacterFactoryV2.loadPet(scene, role.pet);
    }
  ...
  static loadPet(scene: PreloadScene, petId: number) {
    if (CharacterFactoryV2.petData === undefined || !CharacterFactoryV2.petData.has(petId)) {
      console.error('Pet data', CharacterFactoryV2.petData);
      throw Error(`Invalid pet id ${petId}`);
    }

    const { animations } = CharacterFactoryV2.petData.get(petId);
  ...
  static createAnimations(scene: PreloadScene, role: FightRole, avatarAnimations: Set<string> = new Set()) {
    const { animations, weaponAnimations, secretTechnique, name } = CharacterFactoryV2.characterData[role.avatar];
    const weaponId = '' + (role.weaponId ?? 1);
    ...
    if (role.pet) {
      CharacterFactoryV2.createPetAnimations(scene, role.pet);
    }
  }
